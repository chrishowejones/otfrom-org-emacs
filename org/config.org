...
*** fullscreen
    ...
    Want to toggle fulscreen on a keybinding M-f9
...
*** set up erc for slack -DevCycle
    ...
    #+BEGIN_SRC emacs-lisp
      (defun erc-devcycle ()
        (interactive)
         (erc-tls :server "devcycle.irc.slack.com" :full-name "Chris Howe-Jones" :nick "agile_geek" :password "devcycle.i7luSiJiKIPRBy2BYfDr"))
    #+END_SRC
