...
** Easy PG
   ...
   Use Easy PG for opening and encrypting files using
   GPG.

   #+BEGIN_SRC emacs-lisp
;;     (maybe-install-and-require 'epa-file)
;;     (epa-file-enable)
   #+END_SRC
...
*** set up erc for slack - clojurians

    want to use erc to communicate in slack

    #+BEGIN_SRC emacs-lisp
      (defun erc-clojurians ()
        (interactive)
         (erc-tls :server "clojurians.irc.slack.com" :full-name "Chris Howe-Jones" :nick "agile_geek" :password "clojurians.XjjXy2eisasXhgD951G4"))
    #+END_SRC

*** set up erc for slack -DevCycle

    #+BEGIN_SRC emacs-lisp
      (defun erc-devcycle ()
        (interactive)
         (erc-tls :server "devcycleteam.irc.slack.com" :full-name "Chris Howe-Jones" :nick "agile_geek" :password "devcycleteam.AyIa29itQixGtfTUgh6N"))
    #+END_SRC
...
***** Start clj and cljs repls with figwheel-sidecar

      Stole this from [[https://github.com/oliyh/spa-skeleton#clojure-and-clojurescript-side-by-side][oliyh]]
...
**** My initial windows setup
      ...
      Want to open four windows and sw1nn-cider-perspective in one key
      sequence.

       #+BEGIN_SRC emacs-lisp
        (defun chj-clojure-perspective ()
         (interactive)
         (delete-other-windows)
         (progn
               ;; create our 2 columns
               ...
               ;; move to the rightmost and split into 2 vertically
               (window-number-select 3)
               (split-window-below))
        (balance-windows)
        )
        (global-set-key (kbd "s-<f11>") 'chj-clojure-perspective)
       #+END_SRC

**** Expectations mode

     Expectations mode to run tests in expectations.
